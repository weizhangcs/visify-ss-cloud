...
class RefinerySliceRegrouperHandler(BaseTaskHandler):
    """
    [Handler] [Refinery] 场景聚类任务
    """

    def handle(self, task: Task) -> dict:
        self.logger.info(f"🚀 Starting REFINERY_SLICE_REGROUPER Task: {task.id}")

        # 0. 加载配置
        service_config = AIConfigLoader().get_config("slice_regrouper")
        ...
        debug_dir = None
        if is_debug:
            debug_dir = settings.SHARED_LOG_ROOT / f"refinery_slice_regrouper_{task.id}_debug"
        ...
        output_filename = f"refinery_slice_regrouper_result_{task.id}.json"
        output_dir = settings.SHARED_TMP_ROOT / org_id / f"refinery_slice_regrouper_{task.id}_workspace"
