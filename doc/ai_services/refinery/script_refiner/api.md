...
当任务执行失败时，任务状态 (`status`) 将变更为 `FAILED`。
此时，通过查询任务详情接口 (`GET /api/v1/tasks/{task_id}`)，可在返回体的 `error` 字段中获取结构化的异常信息。

本服务遵循 VSS Cloud 标准异常处理机制。关于 `error` 字段的完整结构定义及客户端处理建议，请参阅：
👉 **Task Manager 异常处理指南**

### 本服务常见错误码 (Common Error Codes)

| Code | Description | 触发场景 (Context) |
| :--- | :--- | :--- |
| `PAYLOAD_VALIDATION_ERROR` | 请求参数校验失败 | `payload` 缺少必填字段或类型错误。 |
| `FILE_IO_ERROR` | 文件读写错误 | `input_file_path` 不存在、无法读取或 Prompt 模板缺失。 |
| `LLM_INFERENCE_ERROR` | 模型推理失败 | 重试多次后 LLM 仍无法返回有效结果。 |
| `INTERNAL_ERROR` | 系统内部错误 | 其他未捕获的异常。 |